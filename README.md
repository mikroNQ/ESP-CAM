...
- **`/bmp` медленнее `/capture`** (BMP-конверсия + больше байт): для сбора тянуть `/capture` (JPEG). С PSRAM `/bmp` ~0.8с, `/capture` ~0.3с.
...
- **Подвисания камеры под нагрузкой** были из-за одного кадрового буфера (детектор vs HTTP); **двойная буферизация в PSRAM это убрала** (таймаутов больше нет). Остаточные редкие спайки латентности — от длинной выдержки `aec_value=1300` (режет fps), а не от буфера.
- **Питание с USB-TTL → брауаут.** Регулятор переходника не тянет броски тока камеры+WiFi+PSRAM → ребуты/стопор. Признак: в Serial `[boot] reset_reason=9`, в `/status` сбрасывается `uptime_s`. Лечение — отдельный 5V ≥1А на пин `5V`, GND общий.
...
### Память и буферизация камеры

Камера работает в **RGB565 @ QQVGA (160×120)** — детектору нужны сырые пиксели, а
веб-поток `/stream` и `/capture` кодируют кадр в JPEG на лету. При наличии PSRAM
(на нашем экземпляре ~4.2 МБ) кадровый буфер размещается в ней с **двойной
буферизацией** (`fb_count=2`, `CAMERA_GRAB_LATEST`): детектор и HTTP-обработчики
перестают драться за единственный буфер (это убрало таймауты `/capture` под
нагрузкой), детектор получает свежий кадр без блокировки. Без PSRAM — фолбэк на
один буфер в DRAM. Геометрия (QQVGA) не меняется, чтобы ROI/модель оставались
валидными.
...
Дефолты (endpoint, ROI, экспозиция, частота, дебаунс, размер арены) — в [`CameraWebServer/config.h`](CameraWebServer/config.h). Поля детектора (`det_*`) и **диагностика здоровья** (`uptime_s`, `free_heap`, `min_free_heap`, `rssi`, `reset_reason`) добавлены в `/status` — помогают отличить стопор от ребута (брауаут: `reset_reason=9`). Латентность перехода ~25 мс (`DET_DEBOUNCE_COUNT=1`).
